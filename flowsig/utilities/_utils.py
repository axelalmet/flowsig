...
def get_top_nmf_genes(adata: sc.AnnData,
    ...
    if gene_type == 'tf':
        tfs_list = pd.read_csv('../data/allTFs_' + model_organism + '.txt', header=None)[0].tolist()
    ...
    for gem in gems:
        ...
        gem_index = all_gems.index(all_gems)
...
def get_top_pyliger_genes(adata: sc.AnnData,
    ...
    pyliger_conds = [key for key in adata.uns['pyliger_info'].keys() if key not in ['n_vars', 'vars']]
...
def get_top_gem_genes(adata: sc.AnnData,
    ...
    if method == 'nmf':
          ...
          return get_top_nmf_genes(adata, gems, n_genes, gem_key, gene_type, model_organism)
    
    elif method == 'pyliger':
          
        return get_top_pyliger_genes(adata, gems, n_genes, gem_key, gene_type, model_organism)
    
    else: # Should be NSF
    
        return get_top_nsf_genes(adata, gems, n_genes, gem_key, gene_type, model_organism)
