    ...
    *,
    ...
    for ad in adatas.values():
        if "spatial_connectivities" not in ad.obsp:
            sq.gr.spatial_neighbors(ad, coord_type=coord_type, n_neighs=n_neighs, n_rings=n_rings, library_key=library_key)
        sq.gr.spatial_autocorr(ad, genes=ad.var_names, n_perms=n_perms, n_jobs=n_jobs, backend=backend, show_progress_bar=False)
